...
>(({ className, children, ...props }, ref) => (
	<NavigationMenuPrimitive.Root
		ref={ref}
		className={cn(
			"relative p-2 z-10 w-full flex flex-1 items-center justify-between bg-zinc-50 " +
				"dark:bg-black opacity-90 dark:shadow-white",
			className,
		)}
		{...props}
	>
...
const navigationMenuTriggerStyle = cva(
	"group inline-flex h-10 w-max items-center justify-center rounded-md bg-white px-4 py-2 text-sm font-medium transition-colors " +
		...
		"disabled:pointer-events-none disabled:opacity-50 data-[active]:bg-zinc-100/50 data-[state=open]:bg-zinc-100/50 " +
		"dark:bg-zinc-950 dark:hover:bg-zinc-800 dark:hover:text-zinc-50 dark:focus:bg-zinc-800 dark:focus:text-zinc-50 " +
		"dark:data-[active]:bg-zinc-800/50 dark:data-[state=open]:bg-zinc-800/50 shadow dark:shadow-white",
);
		...
		<NavigationMenuPrimitive.Viewport
			className={cn(
				"origin-top-center relative mt-1.5 h-[var(--radix-navigation-menu-viewport-height)] w-full " +
					"overflow-hidden rounded-md border border-zinc-200 bg-white text-zinc-950 shadow-lg " +
